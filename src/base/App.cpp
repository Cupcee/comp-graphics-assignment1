...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		float base_x0 = FW::cos(angle_increment * i) * radius;
		float base_x1 = FW::cos(angle_increment * (i + 1)) * radius;
		float base_z0 = FW::sin(angle_increment * i) * radius;
		float base_z1 = FW::sin(angle_increment * (i + 1)) * radius;

		v0.position = Vec3f(base_x0, -height, base_z0);
		v1.position = Vec3f(base_x1, -height, base_z1);
		v2.position = v1.position - v0.position;

		Vec3f cross_product1 = FW::cross(v0.position, v1.position);
		Vec3f cross_product2 = FW::cross(v1.position, cross_product1);
		Vec3f normal = FW::normalize(cross_product2);
...
:   common_ctrl_			(CommonControls::Feature_Default & ~CommonControls::Feature_RepaintOnF5),
	...
	model_translation_		{ 0.0f, 0.0f, 0.0f }
{
...
void App::render() {
	...
	Mat4f modelToWorld;
	Mat4f translation = Mat4f::translate(model_translation_);
